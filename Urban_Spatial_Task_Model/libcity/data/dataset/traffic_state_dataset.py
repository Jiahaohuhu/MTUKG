...
#         self.use_static_kg_embedding = self.config.get('use_static_kg_embedding', False)
#         self.use_temporal_kg_embedding = self.config.get('use_temporal_kg_embedding', False)
#         self.static_kg_embedding_path = self.config.get('static_kg_embedding_path', '')
#         self.temporal_kg_embedding_path = self.config.get('temporal_kg_embedding_path', '')
#         self.temporal_kg_index_path = self.config.get('temporal_kg_index_path', '')
#         self.kg_temporal_align = str(self.config.get('kg_temporal_align', 'year')).lower()
#         self.kg_dim = 0
#         static_kg_cache_key = os.path.basename(str(self.static_kg_embedding_path))
#         temporal_kg_cache_key = os.path.basename(str(self.temporal_kg_embedding_path))
#         self.parameters_str = \
...
#             + str(self.use_static_kg_embedding) + '_' + str(self.use_temporal_kg_embedding) + '_' \
#             + static_kg_cache_key + '_' + temporal_kg_cache_key + '_' \
#             + str(self.kg_temporal_align)
...
#     def _infer_kg_city(self):
#         dataset_upper = str(self.dataset).upper()
#         if dataset_upper.startswith('NYC'):
#             return 'NYC'
#         if dataset_upper.startswith('CHI'):
#             return 'CHI'
#         return dataset_upper[:3]

#     def _resolve_kg_path(self, configured_path, filename):
#         if configured_path:
#             return configured_path
#         city = self._infer_kg_city()
#         candidates = [
#             os.path.join('..', 'HUSK_Embedding_Model', 'embedding', filename.format(city=city)),
#             os.path.join('..', 'UrbanKG_Embedding_Model', 'embedding', filename.format(city=city)),
#         ]
...
#     def _load_static_kg_embedding(self):
#         path = self._resolve_kg_path(
#             self.static_kg_embedding_path,
#             '{city}_region_hybrid_embedding.npy',
#         )
#         if not os.path.exists(path):
#             path = self._resolve_kg_path(
#                 self.static_kg_embedding_path,
#                 '{city}_region_embedding.npy',
#             )
#         if not os.path.exists(path):
#             raise FileNotFoundError('Static KG embedding file not found: {}'.format(path))
#         embedding = np.load(path).astype(np.float32)
#         if embedding.ndim != 2:
#             raise ValueError('Static KG embedding must be 2D, got shape {}'.format(embedding.shape))
#         if embedding.shape[0] != self.num_nodes:
#             raise ValueError(
#                 'Static KG embedding node count mismatch: data has {}, embedding has {}'.format(
#                     self.num_nodes, embedding.shape[0]
#                 )
#             )
#         return embedding

#     def _load_temporal_kg_embedding(self):
#         embedding_path = self._resolve_kg_path(
#             self.temporal_kg_embedding_path,
#             '{city}_region_temporal_embedding.npy',
#         )
#         if not os.path.exists(embedding_path):
#             raise FileNotFoundError('Temporal KG embedding file not found: {}'.format(embedding_path))
#         embedding = np.load(embedding_path).astype(np.float32)
#         if embedding.ndim != 3:
#             raise ValueError('Temporal KG embedding must be 3D, got shape {}'.format(embedding.shape))
#         if embedding.shape[1] != self.num_nodes:
#             raise ValueError(
#                 'Temporal KG embedding node count mismatch: data has {}, embedding has {}'.format(
#                     self.num_nodes, embedding.shape[1]
#                 )
#             )

#         index_path = self._resolve_kg_path(
#             self.temporal_kg_index_path,
#             '{city}_region_temporal_index.json',
#         )
#         if not os.path.exists(index_path):
#             raise FileNotFoundError('Temporal KG index file not found: {}'.format(index_path))
...
#     def _timeslot_year(self, timeslot):
#         text = str(timeslot)
#         if len(text) >= 4:
#             year = self._to_int_or_none(text[:4])
#             if year is not None:
#                 return year
#         return None

#     def _timeslot_quarter(self, timeslot):
#         text = str(timeslot)
#         year = self._timeslot_year(timeslot)
#         month = None
#         if len(text) >= 7:
#             month = self._to_int_or_none(text[5:7])
...
#     @staticmethod
#     def _quarter_ordinal(year, quarter):
#         return int(year) * 4 + int(quarter) - 1

#     def _match_temporal_kg_indices(self, metadata, temporal_len, data_len):
#         if self.kg_temporal_align == 'position':
#             return np.minimum(np.arange(data_len, dtype=np.int64), temporal_len - 1)
...
#             idx = self._to_int_or_none(item.get('index'))
#             if idx is None or idx < 0 or idx >= temporal_len:
#                 continue
#             fallback_indices.append(idx)
#             year = self._to_int_or_none(item.get('year'))
#             if year is None:
#                 year = self._to_int_or_none(item.get('time'))
#             if year is not None:
#                 year_to_index[year] = idx
#             quarter = self._to_int_or_none(item.get('quarter'))
#             if year is not None and quarter is not None:
#                 quarter_to_index[(year, quarter)] = idx

#         if not fallback_indices:
#             fallback_indices = list(range(temporal_len))

#         indices = []
#         if self.kg_temporal_align == 'cycle':
#             for offset in range(data_len):
#                 indices.append(fallback_indices[offset % len(fallback_indices)])
#             return np.asarray(indices, dtype=np.int64)

#         if self.kg_temporal_align == 'quarter':
#             if not quarter_to_index:
#                 raise ValueError(
#                     'kg_temporal_align=quarter requires quarter metadata in the temporal KG index. '
#                     'Re-export embeddings with get_embedding.py --temporal_time_granularity quarter.'
#                 )
#             available_quarters = sorted(quarter_to_index.keys())
#             available_ordinals = {
#                 self._quarter_ordinal(year, quarter): (year, quarter)
#                 for year, quarter in available_quarters
#             }
#             for timeslot in self.timesolts:
#                 quarter_key = self._timeslot_quarter(timeslot)
#                 if quarter_key is None:
#                     indices.append(fallback_indices[0])
#                     continue
#                 if quarter_key in quarter_to_index:
#                     indices.append(quarter_to_index[quarter_key])
#                     continue
#                 target = self._quarter_ordinal(quarter_key[0], quarter_key[1])
#                 nearest = min(available_ordinals.keys(), key=lambda item: abs(item - target))
#                 indices.append(quarter_to_index[available_ordinals[nearest]])
#             return np.asarray(indices, dtype=np.int64)

#         available_years = sorted(year_to_index.keys())
#         for timeslot in self.timesolts:
#             year = self._timeslot_year(timeslot)
#             if year is None or not available_years:
#                 indices.append(fallback_indices[0])
#             elif year in year_to_index:
#                 indices.append(year_to_index[year])
#             else:
#                 nearest_year = min(available_years, key=lambda item: abs(item - year))
#                 indices.append(year_to_index[nearest_year])
#         return np.asarray(indices, dtype=np.int64)

#     def _reshape_kg_for_data(self, kg, data):
#         if data.ndim == 3:
#             return kg
#         if data.ndim == 4:
#             _, len_row, len_column, _ = data.shape
#             if kg.shape[1] != len_row * len_column:
#                 raise ValueError(
#                     'KG node count {} cannot reshape to grid {}x{}'.format(
#                         kg.shape[1], len_row, len_column
#                     )
#                 )
#             return kg.reshape((kg.shape[0], len_row, len_column, kg.shape[2]))
#         raise ValueError('KG fusion supports 3D/4D traffic data, got {}D'.format(data.ndim))

#     def _add_kg_information(self, data):
#         if not self.use_static_kg_embedding and not self.use_temporal_kg_embedding:
#             return data

#         kg_list = []
#         data_len = data.shape[0]
#         if self.use_static_kg_embedding:
#             static_kg = self._load_static_kg_embedding()
#             static_kg = np.broadcast_to(static_kg[None, :, :], (data_len, static_kg.shape[0], static_kg.shape[1]))
#             kg_list.append(static_kg)

#         if self.use_temporal_kg_embedding:
#             temporal_kg, metadata = self._load_temporal_kg_embedding()
#             temporal_indices = self._match_temporal_kg_indices(metadata, temporal_kg.shape[0], data_len)
#             kg_list.append(temporal_kg[temporal_indices])

#         kg = np.concatenate(kg_list, axis=-1)
#         self.kg_dim = kg.shape[-1]
#         kg = self._reshape_kg_for_data(kg, data)
#         fused = np.concatenate([data, kg], axis=-1)
#         self._logger.info('Added KG features, kg_dim={}, data shape={}'.format(self.kg_dim, fused.shape))
#         return fused
...
#             df = self._add_kg_information(df)
...
#     def _timeslot_day(self, timeslot):
#         text = str(timeslot)
#         year = self._timeslot_year(timeslot)
#         month = None
#         day = None
#         if len(text) >= 10:
#             month = self._to_int_or_none(text[5:7])
#             day = self._to_int_or_none(text[8:10])
...
#     @staticmethod
#     def _day_ordinal(year, month, day):
...
#             if year is not None and quarter is not None:
#                 quarter_to_index[(year, quarter)] = idx
#             month = self._to_int_or_none(item.get('month'))
#             day = self._to_int_or_none(item.get('day'))
...
#                     pass
#                 else:
#                     day_to_index[(year, month, day)] = idx
...
#         if self.kg_temporal_align == 'day':
#             if not day_to_index:
#                 raise ValueError(
#                     'kg_temporal_align=day requires day metadata in the temporal KG index. '
#                     'Re-export embeddings with get_embedding.py --temporal_time_granularity day.'
#                 )
#             available_days = sorted(day_to_index.keys())
#             available_ordinals = {
#                 self._day_ordinal(year, month, day): (year, month, day)
#                 for year, month, day in available_days
#             }
#             for timeslot in self.timesolts:
#                 day_key = self._timeslot_day(timeslot)
#                 if day_key is None:
#                     indices.append(fallback_indices[0])
#                     continue
#                 if day_key in day_to_index:
#                     indices.append(day_to_index[day_key])
#                     continue
#                 target = self._day_ordinal(day_key[0], day_key[1], day_key[2])
#                 nearest = min(available_ordinals.keys(), key=lambda item: abs(item - target))
#                 indices.append(day_to_index[available_ordinals[nearest]])
#             return np.asarray(indices, dtype=np.int64)
...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.input_window = self.config.get('input_window', 12)
        self.output_window = self.config.get('output_window', 12)
        self.use_static_kg_embedding = self.config.get('use_static_kg_embedding', True)
        self.use_temporal_kg_embedding = self.config.get('use_temporal_kg_embedding', True)
        self.static_kg_embedding_path = self.config.get('static_kg_embedding_path', '')
        self.temporal_kg_embedding_path = self.config.get('temporal_kg_embedding_path', '')
        self.temporal_kg_index_path = self.config.get('temporal_kg_index_path', '')
        self.kg_temporal_align = str(self.config.get('kg_temporal_align', 'year')).lower()
        self.use_kg_gate = self.config.get(
            'use_kg_gate',
            self.config.get(
                'use_static_temporal_kg_gate',
                'kg_event_gate_mode' in self.config or 'kg_event_gate_init' in self.config
            )
        )
        self.kg_gate_mode = str(self.config.get('kg_gate_mode', 'weighted_concat')).lower()
        self.kg_gate_value = self.config.get(
            'kg_gate_value',
            self.config.get('static_temporal_kg_gate', self.config.get('kg_event_gate_init', 0.5))
        )
        self.kg_dim = 0
        self.static_kg_dim = 0
        self.temporal_kg_dim = 0
        static_kg_cache_key = os.path.basename(str(self.static_kg_embedding_path))
        temporal_kg_cache_key = os.path.basename(str(self.temporal_kg_embedding_path))
        self.parameters_str = \
            str(self.dataset) + '_' + str(self.input_window) + '_' + str(self.output_window) + '_' \
            ...
            + str(self.use_static_kg_embedding) + '_' + str(self.use_temporal_kg_embedding) + '_' \
            + static_kg_cache_key + '_' + temporal_kg_cache_key + '_' \
            + str(self.kg_temporal_align) + '_' + str(self.use_kg_gate) + '_' \
            + str(self.kg_gate_mode) + '_' + str(self.kg_gate_value)
        self.cache_file_name = os.path.join('./libcity/cache/dataset_cache/',
                                            'traffic_state_{}.npz'.format(self.parameters_str))
        self.cache_file_folder = './libcity/cache/dataset_cache/'
        ensure_dir(self.cache_file_folder)
    ...
    def _timeslot_month(self, timeslot):
        text = str(timeslot)
        year = self._timeslot_year(timeslot)
        month = None
        if len(text) >= 7:
    ...
    def _match_temporal_kg_indices(self, metadata, temporal_len, data_len):
        ...
        if self.kg_temporal_align == 'month':
            if not month_to_index:
                raise ValueError(
                    'kg_temporal_align=month requires month metadata in the temporal KG index. '
                    'Re-export embeddings with get_embedding.py --temporal_time_granularity month.'
                )
            available_months = sorted(month_to_index.keys())
            available_ordinals = {
                self._month_ordinal(year, month): (year, month)
                for year, month in available_months
            }
            for timeslot in self.timesolts:
                month_key = self._timeslot_month(timeslot)
                if month_key is None:
                    indices.append(fallback_indices[0])
                    continue
                if month_key in month_to_index:
                    indices.append(month_to_index[month_key])
                    continue
                target = self._month_ordinal(month_key[0], month_key[1])
                nearest = min(available_ordinals.keys(), key=lambda item: abs(item - target))
                indices.append(month_to_index[available_ordinals[nearest]])
            return np.asarray(indices, dtype=np.int64)
    ...
    def _get_kg_gate_value(self):
        gate = np.asarray(self.kg_gate_value, dtype=np.float32)
        if gate.ndim != 0:
            raise ValueError('kg_gate_value must be a scalar in [0, 1], got shape {}'.format(gate.shape))
        gate = float(gate)
        if gate < 0.0 or gate > 1.0:
            raise ValueError('kg_gate_value must be in [0, 1], got {}'.format(gate))
        return gate

    def _gate_static_temporal_kg(self, static_kg, temporal_kg):
        if static_kg.shape[:2] != temporal_kg.shape[:2]:
            raise ValueError(
                'Static and temporal KG shape mismatch before gating: {} vs {}'.format(
                    static_kg.shape, temporal_kg.shape
                )
            )

        gate = self._get_kg_gate_value()
        if self.kg_gate_mode in ['weighted_concat', 'concat', 'cat', 'scalar']:
            return np.concatenate([gate * static_kg, (1.0 - gate) * temporal_kg], axis=-1)
        if self.kg_gate_mode in ['weighted_sum', 'sum', 'add', 'blend']:
            if static_kg.shape[-1] != temporal_kg.shape[-1]:
                raise ValueError(
                    'kg_gate_mode={} requires equal static/temporal KG dims, got {} and {}'.format(
                        self.kg_gate_mode, static_kg.shape[-1], temporal_kg.shape[-1]
                    )
                )
            return gate * static_kg + (1.0 - gate) * temporal_kg
        raise ValueError(
            'Unsupported kg_gate_mode {}. Use weighted_concat or weighted_sum.'.format(self.kg_gate_mode)
        )
    ...
    def _add_kg_information(self, data):
        ...
        if self.use_static_kg_embedding:
            static_kg = self._load_static_kg_embedding()
            static_kg = np.broadcast_to(static_kg[None, :, :], (data_len, static_kg.shape[0], static_kg.shape[1]))
            self.static_kg_dim = static_kg.shape[-1]

        if self.use_temporal_kg_embedding:
            temporal_kg, metadata = self._load_temporal_kg_embedding()
            temporal_indices = self._match_temporal_kg_indices(metadata, temporal_kg.shape[0], data_len)
            temporal_kg = temporal_kg[temporal_indices]
            self.temporal_kg_dim = temporal_kg.shape[-1]

        kg_list = []
        if static_kg is not None and temporal_kg is not None and self.use_kg_gate:
            kg = self._gate_static_temporal_kg(static_kg, temporal_kg)
        else:
            if static_kg is not None:
                kg_list.append(static_kg)
            if temporal_kg is not None:
                kg_list.append(temporal_kg)
            kg = np.concatenate(kg_list, axis=-1)
        self.kg_dim = kg.shape[-1]
        kg = self._reshape_kg_for_data(kg, data)
        fused = np.concatenate([data, kg], axis=-1)
        self._logger.info(
            'Added KG features, kg_dim={}, static_kg_dim={}, temporal_kg_dim={}, '
            'use_kg_gate={}, kg_gate_mode={}, kg_gate_value={}, data shape={}'.format(
                self.kg_dim, self.static_kg_dim, self.temporal_kg_dim,
                self.use_kg_gate, self.kg_gate_mode, self.kg_gate_value, fused.shape
            )
        )
        return fused
