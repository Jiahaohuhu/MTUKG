...
class MultiHeadAttentionAwareTemporalContex_qc_kc(nn.Module):  # key causal; query causal;
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False):
        ...
        if query_multi_segment and key_multi_segment:
            ...
            key_h = self.conv1Ds_aware_temporal_context[1](key[:, :, : self.h_length, :].permute(0, 3, 1, 2))[:, :, :,
                    :-self.padding].contiguous().view(nbatches, self.h, self.d_k, N, -1).permute(0, 3, 1, 4, 2)
            key_list.append(key_h)
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False):
        ...
        if query_multi_segment and key_multi_segment:
            ...
            key_h = self.conv1Ds_aware_temporal_context[1](
                key[:, :, :self.h_length, :].permute(0, 3, 1, 2)).contiguous().view(nbatches, self.h, self.d_k, N,
                                                                                    -1).permute(0, 3, 1, 4, 2)
            key_list.append(key_h)
...
class MultiHeadAttentionAwareTemporalContex_qc_k1d(nn.Module):  # query: causal conv; key 1d conv
    ...
    def forward(self, query, key, value, mask=None, query_multi_segment=False, key_multi_segment=False):
        ...
        if query_multi_segment and key_multi_segment:
            ...
            query_h = self.query_conv1Ds_aware_temporal_context(query[:, :, : self.h_length, :].permute(0, 3, 1, 2))[:,
...
class ASTGNNCommon(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        norm_Adj_matrix = torch.from_numpy(self.norm_Adj(self.adj_mx)).type(torch.FloatTensor).to(
            self.device)  # 通过邻接矩阵，构造归一化的拉普拉斯矩阵
        ...
        if aware_temporal_context:  # employ temporal trend-aware attention
            attn_ss = MultiHeadAttentionAwareTemporalContex_q1d_k1d(nb_head, d_model, num_for_predict, kernel_size,
                                                                    dropout=dropout)  # encoder的trend-aware attention用一维卷积
            attn_st = MultiHeadAttentionAwareTemporalContex_qc_k1d(nb_head, d_model, num_for_predict, kernel_size,
                                                                   dropout=dropout)
            att_tt = MultiHeadAttentionAwareTemporalContex_qc_kc(nb_head, d_model, num_for_predict, kernel_size,
        ...
        if SE and TE:
            encode_temporal_position = TemporalPositionalEncoding(d_model, dropout,
                                                                  num_for_predict)  # decoder temporal position embedding
    ...
    def get_label(self, batch):
        return batch['y'].transpose(1, 2).transpose(2, 3)[:, :, :self.output_dim, :].transpose(-1, -2).transpose(-2,
                                                                                                                 -3)  # B N T F
