...
class Datamap(object):
    ...
    """
    def __init__(self,dataset, dataset_root, use_time_interval=False, time_granularity="year"):
        self.dataset = dataset
        self.dataset_root = dataset_root
        self.use_time_interval=use_time_interval
        if time_granularity not in ("year", "month"):
            raise ValueError("Unsupported time_granularity: {}".format(time_granularity))
    ...
    def get_time_info(self, dataset_root=""):
        ...
        for filename in files_to_read:
            with open(os.path.join(dataset_root, filename)) as f:
                ...
                for l in lines:
                    if len(l) == 4:
                        date1 = self.check_date_validity(l[3], is_end=False)
                        date2 = self.check_date_validity(l[3], is_end=True)
                        if date1 != -1:
                            all_years.append(date1)
                        if date2 != -1:
                            all_years.append(date2)
                        if date1 != -1 and date2 != -1:
                            all_intervals.append((date1, max(date1, date2)))
                        ...
                        date2 = self.check_date_validity(l[4], is_end=True)
                        if date2 != -1:
                            all_years.append(date2)
                        ...
                        if date1 >= 0 and date2 >= 0:
                            ...
                            all_intervals.append((date1, self.max_time_value()))  # date1))
                        elif date2 >= 0:
                            all_intervals.append((self.min_time_value(), date2))  # ,date2))
        ...
        all_years.append(self.min_time_value())
        all_years.append(self.max_time_value())
        ...
        all_intervals.append((self.min_time_value(), self.max_time_value()))
    ...
    def create_year2id(self, triple_time, bin_size=300):
        ...
        for k, v in triple_time.items():
            try:
                start = self.check_date_validity(v[0], is_end=False)
                end = self.check_date_validity(v[1], is_end=True)
                # if(len(v)!=1):
                #     end = v[1].split('-')[0]
                # else:
                #     end='####'
    ...
    def check_date_validity(self, date, is_end=False):
        # if DATASET == 'ICEWS':
        ...
        if year.find('#') != -1 or len(year) != YEAR_STR_LEN:
            return -1
        year = int(year)
        ...
        if month.find('#') != -1 or len(month) != 2:
...
class kb(object):
    ...
    def get_date_range(self, fact):
        ...
        start = self.datamap.check_date_validity(t1, is_end=False)
        end = self.datamap.check_date_validity(t2, is_end=True)

        if (start != -1 and end != -1) and (start > end):
            end = start
        if start == -1 and end != -1:
            start = self.datamap.min_time_value()  # self.unk_time_str#end
        elif start != -1 and end == -1:
            end = self.datamap.max_time_value()  # self.unk_time_str#start
        elif start == -1 and end == -1:
            # start = end = self.unk_time_str
            start = self.datamap.min_time_value()
            # end = YEARMAX
            end = self.datamap.min_time_value()

        return start, end

    def get_span_ids(self, start_in, end_in):
        start = self.datamap.check_date_validity(start_in, is_end=False)
        end = self.datamap.check_date_validity(end_in, is_end=True)
        if start == -1:
            start = self.datamap.min_time_value()
        if end == -1:
            end = self.datamap.max_time_value()
        if start > end:
            end = self.datamap.max_time_value()
        ...
        if start == self.datamap.min_time_value():
            start_lbl = 0
        else:
        ...
        if end == self.datamap.max_time_value():
