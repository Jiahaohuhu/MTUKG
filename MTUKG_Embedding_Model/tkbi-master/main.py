...
def main(mode, dataset, dataset_root, save_dir, tflogs_dir, debug, model_name, model_arguments,
         ...
         eval_max_facts=0, time_granularity="year"
         ):
    ...
    if resume_from_save:
        ...
        # build datamap
        datamap = kb.Datamap(dataset, dataset_root, use_time_interval, time_granularity=time_granularity)
    ...
    datamap_filter = kb.Datamap(dataset, dataset_root_filter, use_time_interval=False, time_granularity=time_granularity)
    ...
    if hasattr(scoring_function, "configure_hybrid_bridge"):
        try:
            scoring_function.configure_hybrid_bridge(datamap)
        except Exception as exc:
            print("Warning: failed to configure hybrid bridge:", exc)
    ...
    if mode == 'train':
        ...
        tr = trainer.Trainer(scoring_function, model_arguments, scoring_function.regularizer, loss, optim, dltrain,
                             ...
                             load_to_gpu=has_cuda, eval_max_facts=eval_max_facts)  # 0.01)
        if resume_from_save:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_max_facts', required=False, type=int, default=0,
                        help="limit valid/test facts during evaluation; 0 means full evaluation")
    ...
    parser.add_argument('--time_granularity', required=False, default='year', choices=('year', 'month'),
                        help="temporal granularity for non-ICEWS datasets")
    ...
    main(arguments.mode, arguments.dataset, dataset_root, arguments.save_dir, arguments.tflogs_dir, arguments.debug,
         ...
         arguments.eval_max_facts, arguments.time_granularity)
