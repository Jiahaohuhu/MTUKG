...
class TimePlex_base(torch.nn.Module):
    def __init__(self, entity_count, relation_count, timeInterval_count, embedding_dim, clamp_v=None, reg=2,
                 ...
                 gie_entity_path="", gie_entity_mask_path="", entity_align_path="", context_neighbors_path="", entity_type_path="",
                 anchor_lambda=0.0, context_lambda=0.0, hybrid_loss_scope="batch",
                 allneg_chunk_size=2048, eval_entity_chunk_size=None, use_e2_embeddings=False):
        ...
        # Legacy TimePlex_base allocated E2_* tables, but this implementation does
        # not use them in forward() or in the returned regularizer value.
        self.use_e2_embeddings = bool(use_e2_embeddings)
        if self.use_e2_embeddings:
        ...
        # --- GIE -> TimePlex hybrid bridge --- #
        self.gie_entity_path = gie_entity_path
        self.gie_entity_mask_path = gie_entity_mask_path
        self.entity_align_path = entity_align_path
        self.context_neighbors_path = context_neighbors_path
        self.entity_type_path = entity_type_path
        self.anchor_lambda = float(anchor_lambda)
        self.context_lambda = float(context_lambda)
        self.hybrid_loss_scope = hybrid_loss_scope
        self.allneg_chunk_size = int(allneg_chunk_size)
        self.eval_entity_chunk_size = None if eval_entity_chunk_size in (None, "", 0) else int(eval_entity_chunk_size)
        self.hybrid_ready = False

        self._gie_static_table = None
        self._gie_static_mask = None
        self._shared_static_id_map = None
        self._context_index_map = None
        self._context_sources = None
        self._align_map = None
        self._context_map = None
        self._type_map = None

        self.gie_projection = None
        if self.gie_entity_path and os.path.exists(self.gie_entity_path):
            try:
                static_dim = int(np.load(self.gie_entity_path, mmap_mode="r").shape[1])
                self.gie_projection = torch.nn.Linear(static_dim, 2 * self.embedding_dim, bias=False)
                torch.nn.init.xavier_uniform_(self.gie_projection.weight.data)
                print("Initialized GIE projection with shape: ({}, {})".format(static_dim, 2 * self.embedding_dim))
            except Exception as exc:
                print("Failed to initialize GIE projection from {}: {}".format(self.gie_entity_path, exc))
        # ------------------------------------- #
    ...
    def _load_align_map(self):
        mapping = {}
        if not self.entity_align_path or (not os.path.exists(self.entity_align_path)):
            return mapping

        path = self.entity_align_path
        if path.lower().endswith(".json"):
            with open(path, "r", encoding="utf-8") as handle:
                payload = json.load(handle)
            if isinstance(payload, dict):
                for key, val in payload.items():
                    parsed = self._safe_int(val)
        ...
        with open(path, "r", encoding="utf-8", newline="") as handle:
            ...
            if header is not None:
                has_header = any(self._safe_int(x) is None for x in header)
            if not has_header and header is not None and len(header) >= 2:
                parsed = self._safe_int(header[1])
                if parsed is not None:
                    mapping[str(header[0]).strip()] = parsed
            for row in reader:
                if len(row) < 2:
                    continue
                parsed = self._safe_int(row[1])
                if parsed is not None:
                    mapping[str(row[0]).strip()] = parsed
        return mapping

    def _load_context_map(self):
        mapping = {}
        if not self.context_neighbors_path or (not os.path.exists(self.context_neighbors_path)):
            return mapping

        path = self.context_neighbors_path
        if path.lower().endswith(".json"):
            with open(path, "r", encoding="utf-8") as handle:
                payload = json.load(handle)
            if isinstance(payload, dict):
                for key, val in payload.items():
                    if isinstance(val, list):
                        mapping[str(key)] = [v for v in val]
                    else:
                        mapping[str(key)] = [val]
            return mapping
        ...
        with open(path, "r", encoding="utf-8", newline="") as handle:
            ...
            has_header = False
            if header is not None:
                has_header = any(self._safe_int(x) is None for x in header[:2])
            if (not has_header) and header is not None and len(header) >= 2:
                mapping[str(header[0]).strip()] = str(header[1]).replace("|", " ").replace(",", " ").split()
            for row in reader:
                if len(row) < 2:
                    continue
                mapping[str(row[0]).strip()] = str(row[1]).replace("|", " ").replace(",", " ").split()
        return mapping

    def _load_type_map(self):
        mapping = {}
        if not self.entity_type_path or (not os.path.exists(self.entity_type_path)):
            return mapping

        with open(self.entity_type_path, "r", encoding="utf-8", newline="") as handle:
            reader = csv.DictReader(handle)
            if reader.fieldnames is not None:
                key_col = "entity" if "entity" in reader.fieldnames else (
                    "entity_id" if "entity_id" in reader.fieldnames else reader.fieldnames[0]
                )
                type_col = "type" if "type" in reader.fieldnames else reader.fieldnames[-1]
                for row in reader:
                    key = row.get(key_col)
        ...
        with open(self.entity_type_path, "r", encoding="utf-8") as handle:
            for raw_line in handle:
                parts = raw_line.strip().split()
                if len(parts) < 2:
                    continue
                mapping[str(parts[0]).strip()] = str(parts[1]).strip()
        return mapping

    def _load_static_mask(self, static_count):
        if not self.gie_entity_mask_path or (not os.path.exists(self.gie_entity_mask_path)):
            return None

        path = self.gie_entity_mask_path
        ...
        if values.shape[0] == static_count:
            return values.astype(np.bool_)

        mask = np.zeros(static_count, dtype=np.bool_)
        ids = values.astype(np.int64)
        ids = ids[(ids >= 0) & (ids < static_count)]
        mask[ids] = True
        return mask

    def _is_static_source_valid(self, static_id):
        if static_id is None or static_id < 0:
            return False
        if self._gie_static_mask is None:
            return True
        return static_id < len(self._gie_static_mask) and bool(self._gie_static_mask[static_id])

    def _lookup_static_id(self, token, local_id):
        token_str = str(token)
        if self._align_map:
            if token_str in self._align_map:
                return self._align_map[token_str]
            if str(local_id) in self._align_map:
                return self._align_map[str(local_id)]
        parsed = self._safe_int(token_str)
        return parsed

    def _lookup_context_neighbors(self, token, local_id, static_id):
        if not self._context_map:
            return []
        candidates = [str(token), str(local_id)]
        if static_id is not None and static_id >= 0:
            candidates.append(str(static_id))
        for key in candidates:
            if key in self._context_map:
                neighbors = []
                for value in self._context_map[key]:
    ...
    def _lookup_entity_type(self, token, local_id, static_id):
    ...
    def _init_entity_embeddings_from_sources(self, local_ids, static_sources):
        if len(local_ids) == 0:
            return
        if self.gie_projection is None:
            return

        chunk_size = 10000
        with torch.no_grad():
            for begin in range(0, len(local_ids), chunk_size):
                end = min(len(local_ids), begin + chunk_size)
                ids_chunk = np.asarray(local_ids[begin:end], dtype=np.int64)
                src_chunk = np.asarray(static_sources[begin:end], dtype=np.float32)
                src_tensor = torch.from_numpy(src_chunk)
                projected = self.gie_projection(src_tensor).to(self.E_re.weight.dtype)
                ids_tensor = torch.from_numpy(ids_chunk).long()
                self.E_re.weight.data[ids_tensor] = projected[:, :self.embedding_dim]
                self.E_im.weight.data[ids_tensor] = projected[:, self.embedding_dim:]

    def configure_hybrid_bridge(self, datamap):
        if self.gie_projection is None:
            return
        if datamap is None:
            return
        if not self.gie_entity_path or (not os.path.exists(self.gie_entity_path)):
            return

        self._gie_static_table = np.load(self.gie_entity_path, mmap_mode="r")
        static_count = int(self._gie_static_table.shape[0])
        static_dim = int(self._gie_static_table.shape[1])

        self._align_map = self._load_align_map()
        self._context_map = self._load_context_map()
        self._type_map = self._load_type_map()
        self._gie_static_mask = self._load_static_mask(static_count)

        type_sum = {}
        type_cnt = {}
        for key, etype in self._type_map.items():
            sid = self._safe_int(key)
            if sid is None or sid >= static_count or (not self._is_static_source_valid(sid)):
                continue
            if etype not in type_sum:
                type_sum[etype] = np.zeros(static_dim, dtype=np.float64)
                type_cnt[etype] = 0
            type_sum[etype] += np.asarray(self._gie_static_table[sid], dtype=np.float64)
            type_cnt[etype] += 1
        type_proto = {}
        for etype, total in type_sum.items():
            if type_cnt[etype] > 0:
                type_proto[etype] = (total / type_cnt[etype]).astype(np.float32)

        shared_static_id_map = np.full(self.entity_count, -1, dtype=np.int64)
        context_index_map = np.full(self.entity_count, -1, dtype=np.int64)
        context_sources = []
        init_ids = []
        init_sources = []

        for local_id in range(self.entity_count):
            token = datamap.reverse_entity_map.get(local_id, str(local_id))
            static_id = self._lookup_static_id(token, local_id)

            if static_id is not None and static_id < static_count and self._is_static_source_valid(static_id):
                shared_static_id_map[local_id] = int(static_id)
                init_ids.append(int(local_id))
                init_sources.append(np.asarray(self._gie_static_table[static_id], dtype=np.float32))
                continue

            source = None
            neighbors = self._lookup_context_neighbors(token, local_id, static_id)
            valid_neighbors = [
                nid for nid in neighbors
                if 0 <= nid < static_count and self._is_static_source_valid(nid)
            ]
            if len(valid_neighbors) > 0:
                source = np.asarray(self._gie_static_table[valid_neighbors], dtype=np.float32).mean(axis=0)
                context_index_map[local_id] = len(context_sources)
                context_sources.append(source)
            else:
                etype = self._lookup_entity_type(token, local_id, static_id)
                if etype is not None and etype in type_proto:
                    source = type_proto[etype]

            if source is not None:
                init_ids.append(int(local_id))
                init_sources.append(np.asarray(source, dtype=np.float32))

        self._init_entity_embeddings_from_sources(init_ids, init_sources)

        self._shared_static_id_map = shared_static_id_map
        self._context_index_map = context_index_map
        if len(context_sources) > 0:
            self._context_sources = np.stack(context_sources).astype(np.float32)
        else:
            self._context_sources = np.zeros((0, static_dim), dtype=np.float32)
        self.hybrid_ready = True

        shared_count = int(np.sum(self._shared_static_id_map >= 0))
        context_count = int(np.sum(self._context_index_map >= 0))
        valid_static_count = (
            int(np.sum(self._gie_static_mask)) if self._gie_static_mask is not None else static_count
        )
        print(
            "Hybrid bridge ready | valid_static={} shared={} temporal_only_with_context={} anchor_lambda={} context_lambda={}".format(
                valid_static_count,
                shared_count,
                context_count,
                self.anchor_lambda,
    ...
    def hybrid_auxiliary_loss(self, entity_ids=None):
        device = self.E_re.weight.device
        zero = torch.zeros(1, device=device, dtype=self.E_re.weight.dtype).squeeze()

        if not self.hybrid_ready:
            return zero
        if self.gie_projection is None:
            return zero
        if self.anchor_lambda <= 0.0 and self.context_lambda <= 0.0:
            return zero

        if entity_ids is None or self.hybrid_loss_scope == "all":
            active_ids = np.arange(self.entity_count, dtype=np.int64)
        else:
            active_ids = np.unique(entity_ids.detach().cpu().numpy().astype(np.int64))
        if active_ids.size == 0:
            return zero

        total_loss = zero

        if self.anchor_lambda > 0.0 and self._shared_static_id_map is not None:
            static_ids = self._shared_static_id_map[active_ids]
            valid = static_ids >= 0
            if np.any(valid):
                tp_ids = torch.from_numpy(active_ids[valid]).long().to(device)
                src = np.asarray(self._gie_static_table[static_ids[valid]], dtype=np.float32)
                src_t = torch.from_numpy(src).to(device)
                e_tp = torch.cat([self.E_re(tp_ids), self.E_im(tp_ids)], dim=-1)
                e_proj = self.gie_projection(src_t).to(e_tp.dtype)
                anchor_loss = torch.mean((e_tp - e_proj) ** 2)
                total_loss = total_loss + self.anchor_lambda * anchor_loss

        if self.context_lambda > 0.0 and self._context_index_map is not None and self._context_sources is not None:
            context_indices = self._context_index_map[active_ids]
            valid = context_indices >= 0
            if np.any(valid):
                tp_ids = torch.from_numpy(active_ids[valid]).long().to(device)
                src = np.asarray(self._context_sources[context_indices[valid]], dtype=np.float32)
                src_t = torch.from_numpy(src).to(device)
                e_tp = torch.cat([self.E_re(tp_ids), self.E_im(tp_ids)], dim=-1)
                e_ctx = self.gie_projection(src_t).to(e_tp.dtype)
                context_loss = torch.mean((e_tp - e_ctx) ** 2)
                total_loss = total_loss + self.context_lambda * context_loss

        return total_loss

    def all_neg_obj_ce_loss(self, s, r, o, t, chunk_size=None):
        """
        Memory-safe exact CE over all object entities for TimePlex_base.
        Computes:
            mean_i [ logsumexp_j score(i, j) - score(i, o_i) ]
        without materializing [batch, #entities] score matrix.
        """
        if chunk_size is None:
            chunk_size = self.allneg_chunk_size
        ...
        s_im = self.E_im(s).view(-1, self.embedding_dim)
        s_re = self.E_re(s).view(-1, self.embedding_dim)
        r_im = self.R_im(r).view(-1, self.embedding_dim)
        r_re = self.R_re(r).view(-1, self.embedding_dim)
        rs_im = self.Rs_im(r).view(-1, self.embedding_dim)
        rs_re = self.Rs_re(r).view(-1, self.embedding_dim)
        ro_im = self.Ro_im(r).view(-1, self.embedding_dim)
        ro_re = self.Ro_re(r).view(-1, self.embedding_dim)
        t_im = self.Ts_im(t).view(-1, self.embedding_dim)
        t_re = self.Ts_re(t).view(-1, self.embedding_dim)

        # sro contribution over object candidates
        sro_tmp1 = s_im * r_re + s_re * r_im
        sro_tmp2 = s_re * r_re - s_im * r_im

        # ort contribution over object candidates
        ort_tmp1 = t_im * ro_re + t_re * ro_im
        ort_tmp2 = t_re * ro_re - t_im * ro_im

        # sot contribution over object candidates
        sot_tmp1 = s_im * t_re + s_re * t_im
        sot_tmp2 = s_re * t_re - s_im * t_im

        # Combine query vectors for candidate-dependent part
        q_im = sro_tmp1 + self.ort_wt * ort_tmp1 + self.sot_wt * sot_tmp1
        q_re = sro_tmp2 + self.ort_wt * ort_tmp2 + self.sot_wt * sot_tmp2

        # Candidate-independent srt part
        srt = ((s_re * t_re + s_im * t_im) * rs_re + (s_re * t_im - s_im * t_re) * rs_im).sum(dim=-1)
        const = self.srt_wt * srt

        obj_ids = o.view(-1).long()
        device = q_im.device
        dtype = q_im.dtype
        batch_size = q_im.shape[0]
        num_entities = self.E_re.weight.shape[0]

        running_max = torch.full((batch_size,), -float("inf"), device=device, dtype=dtype)
        running_sumexp = torch.zeros((batch_size,), device=device, dtype=dtype)
        target_score = torch.zeros((batch_size,), device=device, dtype=dtype)

        all_o_re = self.E_re.weight
        all_o_im = self.E_im.weight

        for start in range(0, num_entities, chunk_size):
            end = min(num_entities, start + chunk_size)
            o_re_chunk = all_o_re[start:end]
            o_im_chunk = all_o_im[start:end]

            logits = q_im @ o_im_chunk.transpose(0, 1) + q_re @ o_re_chunk.transpose(0, 1)
            logits = logits + const.unsqueeze(1)

            chunk_max = logits.max(dim=1).values
            new_max = torch.maximum(running_max, chunk_max)
            running_sumexp = (
                running_sumexp * torch.exp(running_max - new_max)
                + torch.exp(logits - new_max.unsqueeze(1)).sum(dim=1)
            )
            running_max = new_max

            mask = (obj_ids >= start) & (obj_ids < end)
            if mask.any():
                rows = torch.nonzero(mask, as_tuple=False).view(-1)
                cols = obj_ids[rows] - start
                target_score[rows] = logits[rows, cols]

        log_denom = running_max + torch.log(running_sumexp + 1e-12)
        return (log_denom - target_score).mean()
...
class TimePlex(torch.nn.Module):
    ...
    def configure_hybrid_bridge(self, datamap):
        if hasattr(self.base_model, "configure_hybrid_bridge"):
            self.base_model.configure_hybrid_bridge(datamap)

    def hybrid_auxiliary_loss(self, entity_ids=None):
        if hasattr(self.base_model, "hybrid_auxiliary_loss"):
            return self.base_model.hybrid_auxiliary_loss(entity_ids)
        return torch.zeros(1, device=self.base_model.E_re.weight.device, dtype=self.base_model.E_re.weight.dtype).squeeze()

    def all_neg_obj_ce_loss(self, s, r, o, t, chunk_size=None):
        if hasattr(self.base_model, "all_neg_obj_ce_loss"):
            return self.base_model.all_neg_obj_ce_loss(s, r, o, t, chunk_size=chunk_size)
        raise NotImplementedError("Base model does not implement all_neg_obj_ce_loss")
