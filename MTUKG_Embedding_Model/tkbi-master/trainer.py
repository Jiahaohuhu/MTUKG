...
class Trainer(object):
    def __init__(self, scoring_function, scoring_function_arguments, regularizer, loss, optim, train, valid, test,
                 ...
                 predict_time=0, time_args=None, flag_add_reverse=0, load_to_gpu=True, eval_max_facts=0):
        ...
        self.eval_max_facts = int(eval_max_facts) if eval_max_facts else 0
    ...
    def step(self):
        ...
        use_chunked_allneg = (
            ...
            and self.flag_add_reverse == 1
            and hasattr(self.scoring_function, "all_neg_obj_ce_loss")
            and self.loss.__class__.__name__ == "crossentropy_loss_AllNeg"
        )

        if use_chunked_allneg:
            fp, fno, fns = None, None, None
            chunk_size = int(self.scoring_function_arguments.get("allneg_chunk_size", 2048))
            loss = self.scoring_function.all_neg_obj_ce_loss(s, r, o, t, chunk_size=chunk_size)
        else:
            need_subject_negatives = self.flag_add_reverse == 0
            if flag_debug:
                fp = self.scoring_function(s, r, o, t, flag_debug=flag_debug + 1)
                fno = self.scoring_function(s, r, no, t, flag_debug=flag_debug + 1)
                fns = self.scoring_function(ns, r, o, t, flag_debug=flag_debug + 1) if need_subject_negatives else None
                ...
                fns = self.scoring_function(ns, r, o, t, flag_debug=0) if need_subject_negatives else None
        ...
        hybrid_loss = None
        if hasattr(self.scoring_function, "hybrid_auxiliary_loss"):
            active_entities = torch.unique(torch.cat([s.view(-1), o.view(-1)], dim=0))
            hybrid_loss = self.scoring_function.hybrid_auxiliary_loss(active_entities)
            if hybrid_loss is not None:
                loss = loss + hybrid_loss

        x = loss.item()
        rg = reg.item() if reg is not None else 0
        hy = hybrid_loss.item() if hybrid_loss is not None else 0
            ...
            if len(losses) >= batch_count[0]:
                ...
                if count == batch_count[1]:
                    ...
                    valid_score = evaluate.evaluate("valid", self.ranker_valid, self.valid.kb, self.eval_batch,
                                                    ...
                                                    flag_add_reverse=self.flag_add_reverse, max_facts=self.eval_max_facts)
