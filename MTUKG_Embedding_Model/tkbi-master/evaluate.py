...
def evaluate(name, ranker, kb, batch_size, predict_time=0, predict_time_pair=0, predict_rel=0, verbose=0,
             hooks=None, save_text=None, load_to_gpu=True, flag_add_reverse=0, max_facts=None):  # True):
    """
    ...
    facts = kb.facts  # [:50]
    if max_facts is not None and int(max_facts) > 0:
        facts = facts[:min(int(max_facts), facts.shape[0])]
        print("Evaluating first {} facts for {}".format(facts.shape[0], name))
    ...
    collect_predictions = save_text is not None
    # predict_rel=False
    ...
    for i in range(0, int(facts.shape[0]), batch_size):
        ...
        if collect_predictions:
            # This block is expensive because it transfers full entity score
            # matrices to CPU and sorts them. Only do it when saving text output.
            num_facts = len(s)
        ...
        if predict_rel and collect_predictions:  # relation scores
