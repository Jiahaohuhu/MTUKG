...
def _resolve_temporal_dataset_path(dataset_path: str, configured_path: str = "") -> str:
    if configured_path:
        return configured_path
    root_temporal = os.path.join(dataset_path, "temporal_train.csv")
    if os.path.exists(root_temporal):
        return dataset_path
    temporal_kg_path = os.path.join(dataset_path, "temporal_kg")
    temporal_kg_train = os.path.join(temporal_kg_path, "temporal_train.csv")
    if os.path.exists(temporal_kg_train):
        return temporal_kg_path
    return dataset_path
...
def _mapping_entity_candidates(value: Any) -> List[str]:
    ...
    if "/" in text:
        ...
        candidates.extend([f"{prefix_lower}::{suffix}", f"{prefix.upper()}:{suffix}"])
        if prefix_lower == "poi":
            candidates.extend([f"point::{suffix}", f"point::static_{suffix}", f"POI:{suffix}", f"POINT:{suffix}"])
        if prefix_lower == "area":
            candidates.extend([f"area::{suffix}", f"AREA:{suffix}", f"Area/{suffix}"])
        if prefix_lower == "road":
            candidates.extend([f"road::{suffix}", f"ROAD:{suffix}", f"Road/{suffix}"])
    elif "::" in text:
        prefix, suffix = text.split("::", 1)
        prefix_lower = prefix.lower()
        candidates.append(f"{prefix.upper()}:{suffix}")
        if prefix_lower == "area":
            candidates.extend([f"AREA:{suffix}", f"Area/{suffix}"])
        if prefix_lower in {"poi", "point"}:
            candidates.extend([f"POI:{suffix}", f"POINT:{suffix}", f"point::static_{suffix}"])
        if prefix_lower == "road":
            candidates.extend([f"ROAD:{suffix}", f"Road/{suffix}"])
    elif ":" in text:
        prefix, suffix = text.split(":", 1)
        prefix_upper = prefix.upper()
        prefix_lower = prefix.lower()
        candidates.append(f"{prefix_lower}::{suffix}")
        if prefix_upper == "AREA":
            candidates.extend([f"area::{suffix}", f"Area/{suffix}"])
        if prefix_upper in {"POI", "POINT"}:
            candidates.extend([f"point::{suffix}", f"point::static_{suffix}", f"POI/{suffix}"])
        if prefix_upper == "ROAD":
            candidates.extend([f"road::{suffix}", f"Road/{suffix}"])
    elif text.isdigit():
        candidates.extend([f"area::{text}", f"AREA:{text}", f"Area/{text}"])
...
def _region_keys(value: Any) -> List[str]:
    ...
    if not text:
        return []
    keys = [text, text.lower(), text.upper()]
    area_id = _parse_area_id(text)
    if area_id is not None:
        keys.extend([str(area_id), f"area::{area_id}", f"AREA:{area_id}", f"Area/{area_id}"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export CHI fused GIE + TimePlex embeddings.")
    parser.add_argument("--dataset", default="CHI", choices=["CHI"])
    parser.add_argument("--data_path", default="data")
    parser.add_argument(
        "--temporal_data_dir",
        default="",
        help="Directory containing temporal_train/valid/test.csv. Defaults to data/CHI; "
             "if absent, falls back to data/CHI/temporal_kg.",
    )
    ...
    parser.add_argument("--output_dir", default="embedding_chi")
    parser.add_argument("--entity_output", default=None, help="Output .npy filename for entity embeddings.")
    parser.add_argument("--region_map_csv", default="areaid2KGid_CHI.csv")
    parser.add_argument(
        "--region_geo_file",
        default="",
        help="Optional LibCity .geo file. If set, region embeddings are exported in this geo_id order.",
    )
    parser.add_argument("--poi_map_csv", default="POIid2KGid_CHI.csv")
    parser.add_argument("--road_map_csv", default="roadid2KGid_CHI.csv")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="PLA,PLR,RRIR",
        help="Comma-separated relation names or ids to keep for CHI event export. "
             "Defaults keep POI temporal changes and road repairs: PLA, PLR, RRIR. "
             "Road-structure-change and land-development relations are not included.",
    )
    parser.add_argument(
        "--event_entity_pattern",
        default=r"^(POI|POINT):commercial|^point::commercial|ROAD_REPAIR_EVENT|road_repair|pothole",
        help="Regex used to identify CHI POI-change and road-repair event entities.",
    )
    parser.add_argument(
        "--event_region_sources",
        default="region,road",
        help="Comma-separated mapping sources for event export. PLA maps POI changes directly to areas; "
             "PLR/RRIR map POI changes or repairs to roads, then roadid2KGid maps roads to areas.",
    )
    parser.add_argument(
    ...
    temporal_dataset_path = _resolve_temporal_dataset_path(dataset_path, args.temporal_data_dir)
    ...
    print(f"Temporal data directory for event export: {temporal_dataset_path}")
    print(f"CHI event relation filter: {args.event_relation_filter}")
